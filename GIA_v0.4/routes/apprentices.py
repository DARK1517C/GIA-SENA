...
def index():
    ...
    return render_template("apprentices/index.html", apprentices=apprentices)
    ...
    if request.method == "POST":
        ...
        try:
            apprentice_count, group_count, has_apprentice, has_group = import_reference_workbook(
                file, owner_id=current_user.id, mode="apprentices"
            )
            flash(f"Importados {apprentice_count} aprendices.", "success")
        except Exception:
...
def export_all():
    try:
        ...
        groups = []  # si quieres exportar grupos aquí puedes importarlos
