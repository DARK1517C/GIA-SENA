    ...
    if apprentice_sheet is not None:
        for index, data in enumerate(parse_apprentice_sheet(apprentice_sheet), start=1):
            ...
            if not group_obj:
                ...
                result.errors.append(f"Aprendiz documento {data.get('document_number') or 'sin documento'} omitido: ficha {group_number or 'sin ficha'} no existe en Record Fichas.")
                continue
