...
def build_evidence_groups(submissions):
    """
    Agrupa submissions por submission.activity.evidence_type.
    Preserva el orden de aparición de los etypes según la lista `submissions`.
    No reordena las claves ni las listas internas.
    """
    groups = OrderedDict()

    for s in submissions:
        etype = getattr(s.activity, "evidence_type", None) or "Sin tipo"
        etype = str(etype).strip()

        if etype not in groups:
            groups[etype] = {"total_files": 0, "approved": 0, "pending": 0, "submissions": []}

        info = groups[etype]

        # construir lista de evidencias para esta submission
        evidences = []
        if getattr(s, "file_name", None) and getattr(s, "file_path", None):
            try:
                download_url = url_for("evidences.download", submission_id=s.id)
            except Exception:
                download_url = None
            ...
            evidences.append({
                ...
                "url": download_url,
                "status": getattr(s, "status_label", None) or getattr(s, "status", None),
                "submitted_at": (s.uploaded_at or s.created_at).strftime("%Y-%m-%d %H:%M") if (s.uploaded_at or s.created_at) else "",
            })
            info["total_files"] += 1
        ...
        if st in ("aprobado",):
            info["approved"] += 1
        elif st in ("pendiente", "pendiente_aprobacion"):
            info["pending"] += 1

        info["submissions"].append({
            "id": s.id,
            "activity": {"id": getattr(s.activity, "id", None), "title": getattr(s.activity, "title", "")},
            "submitted_at": (s.uploaded_at or s.created_at).strftime("%Y-%m-%d %H:%M") if (s.uploaded_at or s.created_at) else "",
            "evidences": evidences,
        })

    # IMPORTANTE: no reordenar ni las claves ni las listas internas
    return groups