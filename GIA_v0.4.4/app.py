...
def create_app(config: dict | None = None) -> Flask:
    ...
    with app.app_context():
        try:
            ...
            auth_bp = apprentices_bp = groups_bp = dashboard_bp = bitacoras_bp = users_bp = evidences_bp = reports_bp = None
            try:
            ...
            try:
                ...
                users_bp = None
            try:
                from routes.evidences import evidences_bp as _evidences_bp  # type: ignore
                evidences_bp = _evidences_bp
            except Exception:
                evidences_bp = None
            try:
                from routes.reports import reports_bp as _reports_bp  # type: ignore
                reports_bp = _reports_bp
            except Exception:
                reports_bp = None
        ...
        try:
            ...
            apprentice_columns = {column["name"] for column in inspector.get_columns("apprentice")}
            schema_changed = False
            apprentice_additions = {
                "followup_instructor_email": "VARCHAR(150)",
                "program_level": "VARCHAR(80)",
                "group_id": "INTEGER",
                "followup_moment1_start": "VARCHAR(40)",
                "followup_moment1_end": "VARCHAR(40)",
                "followup_moment2_start": "VARCHAR(40)",
                "followup_moment2_end": "VARCHAR(40)",
                "followup_moment3_start": "VARCHAR(40)",
                "followup_moment3_end": "VARCHAR(40)",
                "followup_moment4_start": "VARCHAR(40)",
                "followup_moment4_end": "VARCHAR(40)",
                "continues_company": "VARCHAR(30)",
            }
            for column_name, column_type in apprentice_additions.items():
                if column_name not in apprentice_columns:
                    db.session.execute(text(f"ALTER TABLE apprentice ADD COLUMN {column_name} {column_type}"))
                    schema_changed = True
            if schema_changed:
                db.session.commit()
            db.session.execute(text("""
                ...
                SET group_id = (
                    ...
                    FROM training_group
                    WHERE training_group.group_number = apprentice.group_number
                    LIMIT 1
                )
            ...
            app.logger.debug("Schema auto-update skipped.", exc_info=True)
