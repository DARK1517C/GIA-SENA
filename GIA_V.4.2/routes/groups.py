...
def _filtered_groups_query():
    ...
    if quick_search:
        query = query.filter(TrainingGroup.sofia_group_status.ilike(f"%{quick_search}%"))
    return query
...
def bulk_delete():
    ...
    try:
        ...
        db.session.commit()
        flash(f"Eliminadas {deleted} fichas.", "success")
    except Exception:
        db.session.rollback()
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if not has_apprentice and not has_group:
                ...
                flash(f"Importados/actualizados {group_count} fichas y {apprentice_count} aprendices.", "success")
        except Exception:
...
def export_groups():
    try:
        ...
        apprentices = Apprentice.query.filter(Apprentice.group_number.in_(group_numbers)).all() if group_numbers else []
