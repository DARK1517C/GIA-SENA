...
FIELD_SELECT_OPTIONS = {
    ...
    "sofia_status": [
        ...
        ("Habilitado etapa productiva", "Habilitado etapa productiva"),
...
def get_bitacora_or_404(bitacora_id):
    bitacora = Bitacora.query.get_or_404(bitacora_id)
    get_apprentice_or_404(bitacora.apprentice_id)
    return bitacora
...
def dashboard():
    ...
    stats = {
        ...
        "in_training": sum(1 for item in apprentices if "form" in (item.sofia_status or "").lower()),
        "enabled": sum(1 for item in apprentices if "habil" in (item.sofia_status or "").lower()),
        ...
        "with_alternative": sum(
            1 for item in apprentices if item.ep_modality and "sin alternativa" not in (item.individual_management or "").lower()
        ),
        ...
        "operator": sum(1 for item in groups if "oper" in (item.program_level or "").lower()),
        "technical": sum(1 for item in groups if "tecnic" in (item.program_level or "").lower()),
        "technologist": sum(1 for item in groups if "tecnolog" in (item.program_level or "").lower()),
    }
...
@app.route("/bitacoras/<int:bitacora_id>/ver")
@login_required
def bitacora_view(bitacora_id):
    bitacora = get_bitacora_or_404(bitacora_id)
    if not bitacora.file_path or not os.path.exists(bitacora.file_path):
        flash("El archivo de la bitacora no esta disponible.", "error")
        return redirect(url_for("bitacoras"))
    return send_file(bitacora.file_path, as_attachment=False, download_name=bitacora.file_name)


@app.route("/bitacoras/<int:bitacora_id>/descargar")
@login_required
def bitacora_download(bitacora_id):
    bitacora = get_bitacora_or_404(bitacora_id)
    if not bitacora.file_path or not os.path.exists(bitacora.file_path):
        flash("El archivo de la bitacora no esta disponible.", "error")
        return redirect(url_for("bitacoras"))
    return send_file(bitacora.file_path, as_attachment=True, download_name=bitacora.file_name)
...
def bitacora_delete(bitacora_id):
    bitacora = get_bitacora_or_404(bitacora_id)
    apprentice = bitacora.apprentice
    if current_user.role == "visualizador":
        abort(403)
