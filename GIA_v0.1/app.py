...
def build_apprentice_query():
    ...
    if group_filter:
        query = query.filter(Apprentice.group_number == group_filter)
    if modality:
        query = query.filter(Apprentice.ep_modality == modality)
    if status:
...
def build_group_query():
    ...
    if modality:
        query = query.filter(TrainingGroup.modality == modality)
    return query
...
def dashboard():
    if current_user.role == "docente":
        apprentices = Apprentice.query.filter_by(created_by=current_user.id).all()
        groups = TrainingGroup.query.filter_by(created_by=current_user.id).all()
    elif current_user.role == "aprendiz":
        apprentices = Apprentice.query.filter_by(student_user_id=current_user.id).all()
        groups = (
            TrainingGroup.query.filter(
                TrainingGroup.group_number.in_([item.group_number for item in apprentices])
            ).all()
            if apprentices
            else []
        )
    ...
    recent_bitacoras = Bitacora.query.order_by(Bitacora.created_at.desc()).limit(5).all()
...
def apprentices():
    ...
    groups = TrainingGroup.query.order_by(TrainingGroup.group_number.asc()).all()
...
def bitacoras():
    ...
    if selected_apprentice_id:
        ...
        bitacora_records = selected_apprentice.bitacoras
    else:
        apprentice_ids = [item.id for item in apprentices]
        if apprentice_ids:
            bitacora_records = Bitacora.query.filter(Bitacora.apprentice_id.in_(apprentice_ids)).all()
        else:
            bitacora_records = []
...
def profile():
    ...
    return render_template("profile.html", apprentice=apprentice)
