...
def create_app(config: dict | None = None) -> Flask:
    ...
    with app.app_context():
        ...
        try:
            ...
            if "followup_instructor_email" not in apprentice_columns:
                db.session.execute(text("ALTER TABLE apprentice ADD COLUMN followup_instructor_email VARCHAR(150)"))
                schema_changed = True
            if "program_level" not in apprentice_columns:
                db.session.execute(text("ALTER TABLE apprentice ADD COLUMN program_level VARCHAR(80)"))
                schema_changed = True
            if schema_changed:
                db.session.commit()
        except Exception:
